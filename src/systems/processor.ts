...
export const ProcessorSystem: System = {
    filter(entities: Entity[]): Entity[] {
        return entities.filter((entity) => entity.withComponents([ProcessorComponent]))
    },
    update(entities: Entity[], world: World): void {
        for (let i = 0; i < entities.length; i++) {
            ...
            const recipt = processorComponent.slot.getComponent<ReciptComponent>(ReciptComponent);
